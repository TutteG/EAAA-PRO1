...
public class Child {
	...
	// Opgave 5
	// Returnere største forøgelse i vægt, tager ikke højde for eventuelle vægttab
	public double getBiggestWeightGain() {
		for (int i = 0; i < weight.length - 1; i++) {

			if (biggestWeightGain < Math.abs(weight[i] - weight[i + 1])) {
				biggestWeightGain = Math.abs(weight[i] - weight[i + 1]);
			}
		}
